...
class StudyQuestionnaireResponse(models.Model):
    ...
    progress_status = fields.Many2one(
        "study.questionnaire.response.progress.status", string="État d'avancement"
    )
    ...
    study_questionnaire_id = fields.Many2one(
        "study.questionnaire", string="Questionnaire", readonly=True
    )
